	...
	public function dashboard() {
		...
		if (isset($user_role_id)) {
			if(env('DB_CONNECTION', 'sqlsrv') == 'sqlsrv'){
				$menus = DB::select('exec get_user_menu_by_role ' . $user_role_id . ',' . $company_id);
			}elseif(env('DB_CONNECTION', 'sqlsrv') == 'mysql'){
			...
			foreach ($menus as $key => $value) {
				...
				if(env('DB_CONNECTION', 'sqlsrv') == 'sqlsrv'){
					$child_menu = DB::select('exec get_child_menu_list ' . $value->menu_id . ' , ' . $user_role_id . ' ,' . $company_id);
				}elseif(env('DB_CONNECTION', 'sqlsrv') == 'mysql'){
...
*/
		...
		$not_interested = $this->notInterested();
		return view('dashboard',
			compact(
				"totalLeadValue", 'highly_interested', 'unassigned', 'converted', 'conversion_ratio', 'might_invest', 'not_interested'));
	}
	...
	protected function unassigned() {
		...
		if(isset($value) && empty($value)){
			$value = CreateLead::where('lead_assign', 'unassigned')->count();
		}
		return $value;
	}
