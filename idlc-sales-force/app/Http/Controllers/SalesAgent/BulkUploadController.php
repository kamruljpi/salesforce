...
use App\Model\IfaManagement\IfaRegistration as Applicant;
...
class BulkUploadController extends Controller
{
    ...
    public function bulkUploadList(Request $request)
    {
        ...
        return view('sales_agent.bulk_upload.bulkUploadList',['organizations' => $organizations]);
    }

    public function bulkSearchByOrg(Request $request)
    {
        return Applicant::where('organization_name', $request->orgId)->get();
    }

    public function setImageToSystem(Request $request)
    {
...
//                        $newfile = $_SERVER['DOCUMENT_ROOT'] . '/aml/idlc_aml_images/ifa_registrations/' . $request->application_no . '.' . $ext[1];
//                        File::copy($oldfile, $newfile);
//
//                        DB::table('tbl_ifa_registrations')->where('application_no', $request->application_no)->update(['image_ext' => $ext[1]]);
//                    }else{
//                        $return_data_arr = [
//                            'has_error' => TRUE,
...
//            return response()->json($return_data_arr);
        ...
        if($request->upload_picture != ''){
            DB::table('tbl_ifa_registrations')->where('application_no', $request->application_no)->update(['image_ext' => 'png']);
            return "Image has benn succesfully inserted";
        }
        else
            return "Image has not inserted";
    }
}
